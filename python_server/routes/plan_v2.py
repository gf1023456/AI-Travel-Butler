...
async def create_plan_v2(payload: Dict):
    ...
    try:
        message = await call_model_once(provider, model, messages, request_id)
    ...
    if not itinerary:
        raise HTTPException(status_code=502, detail="无法解析模型输出，请重试")

    mcp_trace = [f"plan_v2:{request_id}:parsed:{len(itinerary)}_locations"]
